...
class OpenVLAPPOAlign:
    def __init__(self, all_args, policy: OpenVLAPolicy):
        ...
        if self.args.alg_name not in ['ppo', 'grpo']:
            self.teacher_vision_backbone = self.load_teacher_vision_backbone(self.args.teacher_model_id)
            if self.args.alg_name == 'ppo_alig2':


                in_features = self.policy.vla.vision_backbone.fused_featurizer.patch_embed.proj.base_layer.out_channels
                out_features = self.teacher_vision_backbone.dino_featurizer.patch_embed.proj.out_channels

                projectors_list = nn.ModuleList([
                        AlignmentProjector(in_features, self.args.projector_dim, out_features) for _ in range(len(self.args.align_layers))
                    ]).train()
                
                self.alignment_projector = MultiProjector(projectors_list).to(**self.tpdv).train()
                self.alignment_projector = torch.compile(self.alignment_projector)

                self.params_ap = [p for n, p in self.alignment_projector.named_parameters()]
                self.ap_optimizer = AdamW(self.params_ap, lr=self.args.ap_lr, betas=(self.args.vla_optim_beta1, self.args.vla_optim_beta2))


    def load_teacher_vision_backbone(self, model_id_or_path):
        ...
        if "vision_backbone" in model_state_dict.keys():
                vision_backbone.load_state_dict(model_state_dict["vision_backbone"])

        # pip install "git+https://github.com/TRI-ML/prismatic-vlms"
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        dtype = torch.bfloat16 if device.type == "cuda" else torch.float32
        vision_backbone.to(device, dtype=dtype).eval()
    ...
    def get_teacher_emb(self, pixel_values):

        visual_features = self.teacher_vision_backbone.dino_featurizer.get_intermediate_layers(pixel_values, self.args.align_layers)

        return visual_features

    def get_student_emb(self, obs_image):

        processed_img = self.policy.image_processor(obs_image, return_tensors = TensorType.PYTORCH)
        pixel_values = processed_img['pixel_values'].to(self.tpdv['device'])
        pixel_values_init, pixel_values_fused = torch.split(pixel_values, [3, 3], dim=1)
        dino_features = self.policy.vla.vision_backbone.featurizer.get_intermediate_layers(pixel_values_init, self.args.align_layers)

        if self.args.alg_name == 'ppo_alig2':
            siglip_features = self.policy.vla.vision_backbone.fused_featurizer.get_intermediate_layers(pixel_values_init, self.args.align_layers)
            projected_siglip_features = self.alignment_projector(siglip_features)
            return dino_features, projected_siglip_features, pixel_values_init

        # visual_features = self.policy.vla.vision_backbone(pixel_values)

        return dino_features, pixel_values_init
    ...
    def train_ppo_alig_step(self, idx, total, batch):
        obs_image, instruct, actions, value_preds, returns, masks, old_logprob, advantages = batch
        ...
        if self.args.alg_name == 'ppo_alig':
            # VRA loss DINOv2 on DINOv2
            # ------------------- Student -------------------
            start_s = time.time()
            emb_s, pixel_values = self.get_student_emb(obs['image'].to(**self.tpdv).permute(0, 3, 1, 2).contiguous())
            end_s = time.time()
            # print(f"Student embedding took {end_s - start_s:.4f} seconds")

            # ------------------- Teacher -------------------
            start_t = time.time()
            emb_t = self.get_teacher_emb(pixel_values)
            end_t = time.time()
            # print(f"Teacher embedding took {end_t - start_t:.4f} seconds")

            # ------------------- Alignment loss -------------------
            emb_t = F.normalize(torch.mean(torch.stack(emb_t, dim=0), dim=0), dim=-1)
            emb_s = F.normalize(torch.mean(torch.stack(emb_s, dim=0), dim=0), dim=-1)

            # cossim
            vra_loss = 1.0 - torch.sum(emb_s * emb_t, dim=-1)
            vra_loss = vra_loss.mean()
        elif self.args.alg_name == 'ppo_alig2':
            # VRA loss DINOv2, SigLIP on DINOv2
            # ------------------- Student -------------------
            start_s = time.time()
            emb_s1, emb_s2, pixel_values = self.get_student_emb(obs['image'].to(**self.tpdv).permute(0, 3, 1, 2).contiguous())
            end_s = time.time()
            # print(f"Student embedding took {end_s - start_s:.4f} seconds")
            ...
            # ------------------- Alignment loss -------------------
            emb_t = F.normalize(torch.mean(torch.stack(emb_t, dim=0), dim=0), dim=-1)
            emb_s1 = F.normalize(torch.mean(torch.stack(emb_s1, dim=0), dim=0), dim=-1)
            emb_s2 = F.normalize(torch.mean(torch.stack(emb_s2, dim=0), dim=0), dim=-1)

            # cossim
            cossim_dino = torch.sum(emb_s1 * emb_t, dim=-1)
            cossim_siglip = torch.sum(emb_s2 * emb_t, dim=-1)
            vra_loss = self.args.dino_coef*(1-cossim_dino) + self.args.siglip_coef*(1-cossim_siglip)
            vra_loss = vra_loss.mean()
        ...
        if self.args.alg_name in ['ppo_alig', 'ppo_alig2']:
            loss += self.args.vra_coeff * vra_loss
        ...
        if self.args.alg_name in ['ppo_alig', 'ppo_alig2']:
            info['vra_liss'] = vra_loss.item()
            info['cossim_dino'] = cossim_dino.mean().item()

            if self.args.alg_name == 'ppo_alig2':
                info['cossim_siglip'] = cossim_siglip.mean().item()
    ...
    def train_ppo_alig(self, buffer):
        train_info = defaultdict(lambda: [])
        ...
        for _ in range(self.args.alg_ppo_epoch):
            ...
            for idx, batch in tqdm(enumerate(data_generator), total=minibatch_count, desc="train"):
                info = self.train_ppo_alig_step(idx, minibatch_count, batch)
                for key, value in info.items():
                    train_info[key].append(value)
