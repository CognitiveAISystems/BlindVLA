...
class OpenVLAPPOAlign:
    ...
    def get_teacher_emb(obs_image):

        pixel_values = self.teacher_vlm.vision_backbone.image_transform(obs_image)
        if isinstance(pixel_values, torch.Tensor):
            pixel_values = pixel_values[None, ...].to(self.teacher_vlm.device)
        elif isinstance(pixel_values, dict):
            pixel_values = {k: v[None, ...].to(self.teacher_vlm.device) for k, v in pixel_values.items()}
        else:
    ...
    def get_student_emb(obs_image):

        processed_img = self.image_processor(obs_image, return_tensors = TensorType.PYTORCH)
        pixel_values = processed_img['pixel_values'].to(self.tpdv['device'])
        visual_features = self.vla.vision_backbone(pixel_values)
    ...
    def train_ppo_alig_step(self, idx, total, batch):
        ...
        emb_t = self.get_teacher_emb(torch.tensor(obs_image).to(self.tpdv["device"]))
        emb_s = self.get_student_emb(torch.tensor(obs_image).to(self.tpdv["device"]))
        ...
        # Total loss
        loss = policy_loss + value_loss - self.ppo_entropy_coef * entropy_loss + self.vra_coeff * vra_loss
        loss /= self.args.alg_gradient_accum
        loss.backward()
