...
class OpenVLAPPOAlign:
    def __init__(self, all_args, policy: OpenVLAPolicy):
        ...
        # print('Init Teacher VLM ...')
        # self.teacher_vlm = load("prism-dinosiglip-224px+7b").to(self.tpdv["device"], dtype=torch.bfloat16).eval()
        # for p in self.teacher_vlm.parameters():
        #     p.requires_grad_(False)
        # print('Teacher VLM ready!')
    ...
    # def get_teacher_emb(self, obs_image):

    #     # pixel_values = self.teacher_vlm.vision_backbone.image_transform(obs_image)

    #     pixel_values = [self.teacher_vlm.vision_backbone.image_transform(img) for img in obs_image]

    #     if isinstance(pixel_values[0], torch.Tensor):
    #         pixel_values = torch.stack(pixel_values, dim=0).to(self.teacher_vlm.device)
    #     elif isinstance(pixel_values[0], dict):
    #         pixel_values = {
    #             k: torch.stack([pv[k] for pv in pixel_values], dim=0).to(self.teacher_vlm.device)
    #             for k in pixel_values[0].keys()
    #         }
    #     else:
    #         raise ValueError(f"Unsupported `pixel_values` type = {type(pixel_values)}")
        
    #     autocast_dtype = self.teacher_vlm.llm_backbone.half_precision_dtype
    #     with torch.autocast("cuda", dtype=autocast_dtype, enabled=self.teacher_vlm.enable_mixed_precision_training):
    #         visual_features = self.teacher_vlm.vision_backbone(pixel_values)
    ...
    #     # processed_batch = [
    #     #     self.policy.processor.image_processor(img.convert("RGB"), return_tensors=TensorType.PYTORCH)["pixel_values"]
    #     #     for img in obs_image
    #     # ]
    #     # pixel_values = torch.cat(processed_batch, dim=0).to(self.tpdv["device"])
    ...
    def get_student_emb(self, obs_image):
        ...
        processed_img = self.policy.image_processor(obs_image, return_tensors = TensorType.PYTORCH)
        pixel_values = processed_img['pixel_values'].to(self.tpdv['device'])
        visual_features = self.policy.vla.vision_backbone.featurizer.get_intermediate_layers(pixel_values, self.args.align_layers)
        # visual_features = self.policy.vla.vision_backbone(pixel_values)

        return visual_features, pixel_values
    ...
    def train_ppo_alig_step(self, idx, total, batch):
        ...
        # print(type(obs_image))
        # print(obs_image.shape)

        imgs = [Image.fromarray(im) for im in obs_image]
        tensor_img = torch.stack([torch.tensor(im).to(**self.tpdv) for im in obs_image]).permute((0, 3, 1, 2))

        # emb_s = self.get_student_emb(tensor_img)
        # emb_t = self.get_teacher_emb(imgs)

        # замер для студента
        start_s = time.time()
        emb_s, pixel_values = self.get_student_emb(tensor_img)
        end_s = time.time()
        ...
        cos = (emb_s * emb_t).sum(dim=-1)       
        vra_loss = 1.0 - cos                 
        vra_loss = vra_loss.mean()

        # dbg = {
        #     "vra_cos_mean": cos.mean().item(),
        #     "vra_tokens": S.shape[1],
        #     "vra_dim": S.shape[2],
        # }


        # Total loss
        loss = policy_loss + value_loss - self.ppo_entropy_coef * entropy_loss + self.args.vra_coeff * vra_loss
        loss /= self.args.alg_gradient_accum
        loss.backward()
